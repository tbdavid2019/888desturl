...
function mapSecurity(row) {
  return {
    status: row.security_status || 'unknown',
    source: row.security_source || 'google_webrisk',
    checked_url: row.security_checked_url || null,
    checked_at: row.security_checked_at || null,
    message: row.security_message || null,
...
function mapHistoryRow(row) {
  ...
  const previewUrl = toPublicPreviewUrl(row.preview_path);
  const security = mapSecurity(row);

  return {
    id: row.id,
    result_id: row.public_result_id || null,
    ...
    security_checked_url: security.checked_url,
    security_checked_at: security.checked_at,
    threat_types: security.threat_types,
    security,
...
async function createHistoryStore(options) {
  ...
  async function initialize() {
    await run(
      ...
      `CREATE TABLE IF NOT EXISTS history (
        ...
        security_source TEXT NOT NULL DEFAULT 'google_webrisk',
    ...
    await ensureColumn(db, 'history', 'public_result_id', 'TEXT');
    await ensureColumn(db, 'history', 'chain_json', 'TEXT');
    await ensureColumn(db, 'history', 'security_source', "TEXT NOT NULL DEFAULT 'google_webrisk'");
    await ensureColumn(db, 'history', 'security_checked_at', 'TEXT');
    ...
    await run(
      db,
      'CREATE UNIQUE INDEX IF NOT EXISTS idx_history_public_result_id ON history(public_result_id)'
    );

    const rowsMissingResultId = await all(
      db,
      'SELECT id FROM history WHERE public_result_id IS NULL OR public_result_id = ""'
    );
    for (const row of rowsMissingResultId) {
      await run(db, 'UPDATE history SET public_result_id = ? WHERE id = ?', [
        generateResultId(),
        row.id
      ]);
    }
  ...
  async function recordTrace(entry) {
    const resultId = entry.result_id || generateResultId();
    const insertResult = await run(
      ...
      [
        ...
        entry.security_source,
        entry.security_message,
        entry.security_checked_url,
        entry.security_checked_at,
        entry.threat_types.join(','),
        entry.loop_detected ? 1 : 0,
        JSON.stringify(Array.isArray(entry.chain) ? entry.chain : [])
      ]
    );

    const insertedRow = await get(db, 'SELECT * FROM history WHERE id = ?', [insertResult.lastID]);
    return mapHistoryRow(insertedRow);
  }
  ...
  async function getResultById(resultId) {
    const row = await get(db, 'SELECT * FROM history WHERE public_result_id = ?', [resultId]);
